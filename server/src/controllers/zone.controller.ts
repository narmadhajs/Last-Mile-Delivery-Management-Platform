  ...
  public static async updateZone(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const zone = await ZoneService.updateZone(id, {
        ...data,
        centerLat: data.centerLat !== undefined ? parseFloat(data.centerLat) : undefined,
        centerLng: data.centerLng !== undefined ? parseFloat(data.centerLng) : undefined,
        radiusKm: data.radiusKm !== undefined ? parseFloat(data.radiusKm) : undefined,
      });
